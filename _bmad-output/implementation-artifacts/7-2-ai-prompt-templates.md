...
- [ ] Task 1: Create prompt template directory structure and index (AC: #1, #2)
  - [ ] Create `docs/ai-knowledge-bundle/prompts/` directory
  - [ ] Create `docs/ai-knowledge-bundle/prompts/index.md` with template inventory and usage guide
  - [ ] Update `docs/ai-knowledge-bundle/index.md` to reference the prompts section

- [ ] Task 2: Create "New Module" prompt template (AC: #1, #3)
  - [ ] Create `docs/ai-knowledge-bundle/prompts/new-module.md`
  - [ ] Include input parameters section: module name (kebab-case), domain description, entity list (PascalCase names, e.g., "OrderItem" not "orderItem"), command list, projection list
  - [ ] Include "Clarifying Questions" section: instruct AI to ask user about ambiguous domain aspects before generating (e.g., status fields and valid values, optional vs required fields, entity relationships, date/timestamp fields)
  - [ ] Include reserved module name list: `shell`, `shell-api`, `cqrs-client`, `ui`, `tsconfig`, `eslint-config` cannot be used
  - [ ] Include manifest generation instructions referencing `manifest-schema.json` and `manifest-guide.md`
  - [ ] Include Zod schema generation instructions with domain-specific field patterns
  - [ ] Include domain modeling guidance: how to handle nested entities (separate schemas), one-to-many relationships (array fields in detail schema), optional fields (`z.optional()`), and status/enum fields (`z.union([z.literal(...)])`)
  - [ ] Include page generation instructions for list, detail, and create pages — code-first with code skeletons, not prose-first
  - [ ] Include hook integration instructions (useQuery, useCommandPipeline, useToast)
  - [ ] Include destructive operation guidance: delete/remove commands must use `<AlertDialog>` for confirmation before sending
  - [ ] Include test generation instructions with MockQueryBus/MockCommandBus patterns
  - [ ] Include routes.tsx and index.ts generation with lazy loading
  - [ ] Include dev-host mockSetup.ts generation with sample data
  - [ ] Include CSS Module file generation with design tokens only
  - [ ] Embed quality rules: loading/error/empty states, no inline styles, no direct Radix, no enums
  - [ ] Embed quality checklist items: all user-facing text (page titles, toasts, empty states, button labels) must use domain-specific language — not generic placeholders like "Item created" or "No items yet"
  - [ ] Embed quality checklist item: every route in manifest.ts must have a matching lazy import in routes.tsx (manifest-to-routes consistency)
  - [ ] Embed quality checklist item: CSS class names in `.module.css` files must be camelCase (e.g., `.statusBadge` not `.status-badge`)
  - [ ] Include a "Limitations" note: these templates generate entity-based CRUD modules only; dashboards, wizards, file uploads, multi-step workflows, and real-time streaming views are not covered
  - [ ] Include a complete worked example: full "Input → Output" demonstration (e.g., given "Task management with tasks and projects," show the complete generated manifest, schemas, one page, and one test file)

- [ ] Task 3: Create "Add Command" prompt template (AC: #2, #3)
  - [ ] Create `docs/ai-knowledge-bundle/prompts/add-command.md`
  - [ ] Include input parameters: command name, fields/payload, domain name, target module path
  - [ ] Include "Clarifying Questions" section: instruct AI to ask about field types, validation rules, and whether the command is destructive
  - [ ] Include Zod command schema generation instructions
  - [ ] Include form page generation with useCommandPipeline, Form, FormField, status feedback — code-first with code skeleton
  - [ ] Include destructive operation guidance: if command is destructive (delete, remove, disable), wrap trigger in `<AlertDialog>` confirmation
  - [ ] Include route addition instructions (manifest routes + routes.tsx + lazy import)
  - [ ] Include test generation for the form page
  - [ ] Include index.ts re-export updates

- [ ] Task 4: Create "Add Projection Page" prompt template (AC: #2, #3)
  - [ ] Create `docs/ai-knowledge-bundle/prompts/add-projection.md`
  - [ ] Include input parameters: projection name, fields, domain, query type, target module path
  - [ ] Include Zod schema generation (list item + detail schemas)
  - [ ] Include list page generation with Table, useQuery, column definitions
  - [ ] Include detail page generation with DetailView, useQuery with aggregateId
  - [ ] Include route additions and lazy imports
  - [ ] Include test generation for both pages

- [ ] Task 5: Add template completeness check to freshness script (AC: #4)
  - [ ] Extend `scripts/check-bundle-freshness.ts` to verify all 3 prompt template files exist
  - [ ] Verify each template references current hook names (useSubmitCommand, useCommandPipeline, useQuery)
  - [ ] Verify each template references current component names from @hexalith/ui
  - [ ] Verify hook parameter signatures in template code examples match actual hook exports (not just name presence — check that `useQuery` examples show `(schema, params, options)` shape, `useCommandPipeline` examples show `{ send, status, error }` return shape)
  - [ ] Add WARN if template files haven't been updated since last bundle version bump

- [ ] Task 6: Update documentation cross-references (AC: #1, #4)
  - [ ] Update `docs/module-development.md` to reference prompt templates
  - [ ] Update `docs/ai-knowledge-bundle/index.md` with prompts section entry
  - [ ] Update `CLAUDE.md` if needed for prompt template awareness

- [ ] Task 7: Final verification (AC: #1, #2, #3, #4)
  - [ ] Run `pnpm check:bundle-freshness` — must pass with template checks
  - [ ] Verify each template references the knowledge bundle sections correctly
  - [ ] Verify no existing tests regressed
  - [ ] Verify templates produce code matching scaffold template patterns exactly
...
export const manifest: ModuleManifest = {
  ...
  navigation: [{ label: "{Display Name}", path: "/", icon: "{icon}", category: "Modules" }],
};
```
