...
- [ ] Task 1: Create Getting Started guide (AC: #1, #3, #4)
  - [ ] 1.1: Create `docs/getting-started.md` with these sections:
    - **Prerequisites:** Node.js 22+, pnpm 9+, Git. How to request `@hexalith` GitHub Packages scope access (`.npmrc` configuration for `@hexalith:registry=https://npm.pkg.github.com`)
    ...
    - **Integrate with shell:** `git init && git add -A && git commit`, push to GitHub, shell team runs `git submodule add <repo-url>` to compose
  - [ ] 1.2: All code examples MUST be extracted from or match exactly the scaffold template files in `tools/create-hexalith-module/templates/module/` — no hand-written snippets

- [ ] Task 2: Create Module Development Lifecycle guide (AC: #2, #3, #4)
  - [ ] 2.1: Create `docs/module-development.md` with these sections:
    ...
    - **Shell integration via git submodule:** Module repo pushed to GitHub → shell team adds via `git submodule add` → shell CI validates manifest → shell build includes module routes and navigation

- [ ] Task 3: Create CQRS for Frontend Developers reference (AC: #2, #3)
  - [ ] 3.1: Create `docs/cqrs-frontend-guide.md` with these sections:
    - **What are commands?** A command tells the backend to *do* something (create, update, delete). Frontend sends commands via `useSubmitCommand` or `useCommandPipeline`. The hook abstracts the HTTP POST, auth header injection, correlation ID, and status polling
...
- [ ] Task 4: Create API Quick Reference (AC: #2, #3)
  - [ ] 4.1: Create `docs/api-reference.md` — concise reference of the three package APIs. **Format:** grouped headings per package, with a table per category showing export name, type (component/hook/type/class), and one-line description. MUST list ALL exports from each package barrel (`src/index.ts`), not a curated subset:
...
- [ ] Task 5: Create docs index / table of contents (AC: #1, #2)
  - [ ] 5.1: Create `docs/index.md` — entry point linking all documentation files with brief descriptions:
    ...
    - Link to Storybook for interactive component documentation

- [ ] **DEFINITION OF DONE GATE — All previous tasks must pass these verification checks before the story is complete.**

- [ ] Task 6: Verification (AC: #1-#4)
  - [ ] 6.1: All code examples in `docs/getting-started.md` match scaffold template files (diff check). Trimmed excerpts are acceptable — use `// ...` to indicate omitted lines and cite the source file (e.g., `// See full example: src/pages/ExampleListPage.tsx`). The shown code must be verbatim from the source, not rewritten.
  - [ ] 6.2: All code examples in `docs/module-development.md` match actual source files or scaffold templates (same trimmed-excerpt policy)
  - [ ] 6.3: All CQRS hook names, types, and parameters in `docs/cqrs-frontend-guide.md` match `packages/cqrs-client/src/index.ts` exports
  - [ ] 6.4: All shell-api exports listed in `docs/api-reference.md` match `packages/shell-api/src/index.ts` — no missing exports, no fabricated exports
  - [ ] 6.5: All cqrs-client exports listed in `docs/api-reference.md` match `packages/cqrs-client/src/index.ts` — no missing exports
  - [ ] 6.6: All UI exports listed in `docs/api-reference.md` match `packages/ui/src/index.ts` — no missing exports
  - [ ] 6.7: The Getting Started guide steps can be followed sequentially without gaps (manual walkthrough)
  - [ ] 6.8: No documentation references `react-router-dom` — must use `react-router` (v7 unified package)
  - [ ] 6.9: No documentation references `useCommand` or `useProjection` as hook names — actual names are `useSubmitCommand`/`useCommandPipeline` and `useQuery`
  - [ ] 6.10: `SubmitCommandInput` shape uses `commandType`, `domain`, `payload` — NOT `commandName`, `aggregateName`, `body`
  - [ ] 6.11: Verify `docs/` directory is created at project root with all 5 files (index.md + 4 guides)
  - [ ] 6.12: `docs/index.md` links to all 4 documentation files with correct relative paths
...
1. **(CRITICAL) Code examples MUST be extracted from tested source.** The PRD and DX NFRs specify: "All code examples in docs are extracted from tested source. Stale examples are a P1 bug." This means examples should be copied from `tools/create-hexalith-module/templates/module/` or `packages/*/src/` — not hand-written. Trimmed excerpts are acceptable: use `// ...` to indicate omitted lines and cite the source file (e.g., `// See full example: src/pages/ExampleListPage.tsx`). The shown lines must be verbatim from the source, not rewritten or fabricated. [Source: prd.md#Developer Documentation, architecture.md#DX NFR]
...
8. **Do NOT over-document component props.** Storybook is the living component catalog (Story 3.9). The docs reference Storybook for detailed component documentation.
