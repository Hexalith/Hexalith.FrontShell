...
5. **AC5 — Consumer Contract Tests at CQRS Boundary**
   ...
   - **Then** HTTP-level consumer contracts verify:
     ...
     - `GET /api/commands/{id}` expects status polling response `{ status, events? }`
...
- [ ] **Task 1: Create ATDD compliance checker script** (AC: #1)
  - [ ] 1.1 Create `scripts/check-atdd-compliance.sh`:
  ...
  - [ ] 1.2 Add CI step to `.github/workflows/ci.yml`:
    ```yaml
...
- [ ] **Task 2: Create test quality standards validator** (AC: #4)
  - [ ] 2.1 Create `scripts/check-test-quality.ts`:
  ...
  - [ ] 2.2 Add `check-test-quality` script to root `package.json`:
    ```json
    "check:test-quality": "tsx scripts/check-test-quality.ts"
    ```
  - [ ] 2.3 Add CI step to `.github/workflows/ci.yml`:
    ```yaml
...
- [ ] **Task 3: Document traceability approach** (AC: #3)
  - [ ] 3.1 Add a **Traceability** section to `docs/testing-strategy.md` (Task 4) with:
  ...
  - [ ] 3.2 **DO NOT create a `scripts/traceability-matrix.ts` file** — defer the full script to a future story. The AC marker convention (Task 7) and the documented grep command are sufficient for v1.
  - [ ] 3.3 **DO NOT add to CI** — traceability is an on-demand check, not a PR gate.

- [ ] **Task 4: Create testing strategy documentation** (AC: #2)
  - [ ] 4.1 Create `docs/testing-strategy.md`:
...
- [ ] **Task 5: Create API-level consumer contract tests** (AC: #5)
  ...
  - [ ] 5.2 Create `packages/cqrs-client/src/contracts/commandApi.contract.test.ts`:
    - Define the frontend's expected HTTP contract for command submission:
      ...
      describe('CommandApi Consumer Contract', () => {
        ...
        describe('GET /api/commands/{id} (status polling)', () => {
    ...
    - **Mocking approach:** Use `vi.mock('ky')` to mock the HTTP client at module level. The real bus implementations (`DaprCommandBus`, `DaprQueryBus`) use `ky` via `createKyInstance()`. Mock `ky`'s `post()`, `get()` methods to capture request shapes and return controlled responses. DO NOT mock `createKyInstance` directly — mock at the `ky` boundary so contract tests validate the full request construction path inside the bus implementation.
      ...
      vi.mock('ky', () => ({
        default: {
          create: () => ({
            post: vi.fn().mockResolvedValue({ json: () => Promise.resolve({ correlationId: '...' }) }),
            get: vi.fn().mockResolvedValue({ json: () => Promise.resolve({ status: 'completed' }) }),
          }),
        },
      }));
  ...
  - [ ] 5.3 Create `packages/cqrs-client/src/contracts/queryApi.contract.test.ts`:
    - Define the frontend's expected HTTP contract for projection queries:
      ...
      ```
  - [ ] 5.4 **DO NOT export these contracts from `@hexalith/cqrs-client/testing`** — API-level contracts are internal validation, not reusable fixtures. The bus-level contracts in `mocks/__contracts__/` are the reusable API.
  - [ ] 5.5 **Checkpoint:** Run `pnpm turbo test --filter=@hexalith/cqrs-client` — verify all existing + new contract tests pass

- [ ] **Task 6: Add contract verification CI gate** (AC: #6)
  - [ ] 6.1 Update `.github/workflows/ci.yml` to add a dedicated contract test step:
    ```yaml
  ...
  - [ ] 6.2 If Vitest project filtering doesn't work with `--project cqrs-client`, use `--testPathPattern` instead:
    ```yaml
    run: pnpm --filter @hexalith/cqrs-client exec vitest run --testPathPattern='contracts/'
    ```

- [ ] **Task 7: Add AC markers to existing critical test files** (AC: #3)
  - [ ] 7.1 Add `// AC: story-id#criterion` comments to the most critical existing test files to seed the traceability matrix. **Skip any files that don't exist yet** — stories 6-4 and 6-5 may still be in progress. Those files will get AC markers when their stories are implemented. Target at minimum (if file exists):
    ...
    - `apps/shell/e2e/tenants-navigation.spec.ts` — `// AC: 6-4#7` *(may not exist if 6-4 incomplete)*
    - `apps/shell/e2e/tenants-create.spec.ts` — `// AC: 6-4#7` *(may not exist if 6-4 incomplete)*
  - [ ] 7.2 **Convention:** Place AC markers at the top of the file (after imports) or on the `describe()` block, NOT on individual `it()` blocks (too granular, creates maintenance burden):
    ```typescript
    // AC: 6-3#2 — Tenant list page renders table with data
    describe('TenantListPage', () => { ... });
    ```
  - [ ] 7.3 **IMPORTANT:** Do NOT modify test logic — only add comments. This is a zero-risk change.

- [ ] **Task 8: Update CI pipeline with all new steps** (AC: #1, #4, #6)
  - [ ] 8.1 Final CI step ordering in `.github/workflows/ci.yml`:
    ```
    ...
    ```
  - [ ] 8.2 **Checkpoint:** Verify the CI YAML is valid:
...
- [ ] **Task 9: Final verification** (AC: #1-#6)
  - [ ] 9.1 Run `pnpm turbo build` — verify full workspace builds cleanly
  - [ ] 9.2 Run `pnpm turbo test` — verify all existing + new tests pass
  - [ ] 9.3 Run `pnpm turbo lint` — verify no lint errors
  - [ ] 9.4 Run `bash scripts/check-atdd-compliance.sh` locally — verify it produces expected output
  - [ ] 9.5 Run `pnpm check:test-quality --all` — verify it scans test files and reports results
  - [ ] 9.6 Run the traceability grep command from `docs/testing-strategy.md` — verify AC markers from Task 7 appear in the output
  - [ ] 9.7 **Self-test the scripts** — run `pnpm check:test-quality --all` and verify at least one existing violation is reported (confirms the scanner is working, not silently passing everything). If all files pass cleanly, temporarily add a `setTimeout(() => {}, 100)` to any test file, re-run, verify it's flagged, then revert. Similarly, test the ATDD checker by verifying it correctly identifies a source file without a corresponding test in the current diff.
  - [ ] 9.8 Verify all new scripts are executable: `chmod +x scripts/check-atdd-compliance.sh`
  - [ ] 9.9 Verify no regressions — all pre-existing CI gates still pass
